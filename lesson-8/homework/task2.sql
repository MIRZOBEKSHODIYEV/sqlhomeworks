...
AllYears AS (
    ...
    UNION ALL
    SELECT YearValue + 1 FROM AllYears WHERE YearValue < YEAR(GETDATE()) 
),
MissingYears AS (
    SELECT YearValue FROM AllYears 
    WHERE YearValue NOT IN (SELECT HireYear FROM HireYears)
),
GroupedYears AS (
    SELECT YearValue, 
           YearValue - ROW_NUMBER() OVER (ORDER BY YearValue) AS GroupID
    FROM MissingYears
)
